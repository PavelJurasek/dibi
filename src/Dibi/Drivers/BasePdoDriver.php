 ...
 *   - version
 */
abstract class BasePdoDriver implements Dibi\Driver
{
	use Dibi\Strict;
	...
	public function __construct(array $config)
	{
		...
		if ($config['resource'] instanceof PDO) {
			...
			if ($this->connection->getAttribute(PDO::ATTR_ERRMODE) === PDO::ERRMODE_SILENT) {
				throw new Dibi\DriverException('Provided PDO connection is in unsupported SILENT mode.');
			}

			if ($this->connection->getAttribute(PDO::ATTR_ERRMODE) !== $this->getErrmode()) {
				throw new Dibi\DriverException('Provided PDO connection is not in expected mode.');
			}
		} else {
			try {
				$this->connection = new PDO($config['dsn'], $config['username'], $config['password'], $config['options']);
				$this->connection->setAttribute(PDO::ATTR_ERRMODE, $this->getErrmode());
		...
		$this->serverVersion = (string) ($config['version'] ?? $this->initServerVersion());
	}


	abstract protected function initServerVersion(): ?string;
	 ...
	 */
	abstract public function query(string $sql): ?Dibi\ResultDriver;


	abstract protected function getErrmode(): int;


	protected function createException(array $errorInfo, string $sql): Dibi\DriverException
	{
	 ...
	 * @throws Dibi\DriverException
	 */
	abstract public function getInsertId(?string $sequence): ?int;
	 ...
	 * @throws Dibi\DriverException
	 */
	abstract public function begin(?string $savepoint = null): void;
	 ...
	 * @throws Dibi\DriverException
	 */
	abstract public function commit(?string $savepoint = null): void;
	 ...
	 * @throws Dibi\DriverException
	 */
	abstract public function rollback(?string $savepoint = null): void;
